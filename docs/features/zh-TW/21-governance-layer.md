...
DuDuClaw 的治理層就是這兩道閘門，立在每個 Agent 之前。**Permission 與 Lifecycle 政策**是建築法規：Agent 可以做什麼、什麼需要核准。**Rate 與 Quota 政策**是錶，量出 Agent 在被限速或切斷前可以消耗多快、多少。兩者皆以純 YAML 宣告，皆於修改時熱重載，且當規則手冊缺失或格式錯誤時皆 fail closed。
