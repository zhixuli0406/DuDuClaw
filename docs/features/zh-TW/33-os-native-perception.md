...
- **視窗標題永不寫入磁碟。** 每日 log 只存 app 名稱 + 時間戳,這是關懷摘要需要的最小集合。
...
過去要寫一條 OS 規則,得先背事件名稱、手寫 condition JSON,所以沒人寫。OS 頁現在提供範本卡片(`OsAutomationTemplates.tsx`):**檔案範本**(「這個資料夾有東西進來時,行動」,並同時透過 `os.settings.update` 追加監看路徑),與 **app 範本**(「這個 app 到前景時,在這個通道提醒我」)。各自填一張小表單,經正常的 `autopilot.create` RPC 建立真正的 autopilot 規則,具備伺服器端驗證、斷路器保護,與任何手寫規則無異。
