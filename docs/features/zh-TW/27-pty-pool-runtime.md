...
為了更強的隔離，這個池可以**移到 process 之外**，存在於獨立的 `duduclaw-cli-worker` 子 process 中，由 `[runtime] worker_managed = true` 控制。Gateway 的 `worker_supervisor` 掌管其生命週期。最關鍵的是，它把關閉動作排入 gateway 的優雅關閉流程：
