...
而且有時候他不需要重寫整齣戲，只做一次快速的**片段修正**：調整沒接住的那一小段，在集與集之間直接上線。
...
關鍵創新在於 Verifier 如何與 Generator 溝通：它給的是**具體、可執行的回饋**，不是像「7/10」這樣的單純評分：
...
GVU² 將 prompt 最佳化從手動、容易出錯的過程轉變為自動、安全、高效的管線。雙迴圈架構把策略性成長（行為 GVU）與戰術性修正（任務 GVU）分開，而 MistakeNotebook 確保 Agent 永遠不會忘記自己過去的失敗。Agent 不只是*運行*而已，它同時在*成長*、從錯誤中學習，並帶著耐性演化。
