...
名冊裡每個成員帶 `name`、`role`、`reports_to`(包內主管)、`department`、`rank`、一個觸發關鍵詞,以及一份 skill 清單。`duduclaw expert pack` 在打包前嚴格驗證(slug 形狀、重複名稱、以 topological sort 檢查 `reports_to` 循環、缺 `soul.md`、SKILL.md frontmatter 的 name 必須等於其目錄名);`install` 則寬鬆,回報問題而不是猜著修。`[expert.requires]` 以 doctor 風格列出環境變數與二進位——缺了會警告,絕不擋下安裝。
