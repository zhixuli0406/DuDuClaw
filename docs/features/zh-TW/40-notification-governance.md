...
> 每則推播先標級別，勿擾時段把不急的延後，每天彙整一則摘要，並量測每類通知到底有沒有人理。
...
每則通知從產生到送達（或被延後）走這條路：

```
通知產生（NotifyLevel 必填，沒有預設值）
        |
        v
   是 L3 嗎？ --是--> 立即投遞（勿擾時段不擋 L3）
        |
        否
        |
        v
  勿擾時段生效中？ --否--> 立即投遞
        ...
        |
        v
  寫入 ~/.duduclaw/notify_queue.jsonl（附檔案鎖）
        |
        v
  時段結束後由背景排程器投遞：
    - 同一收件目的地的 L1 合併成一則
...
格式錯誤、空字串、`start == end`（例如 `00:00-00:00`）全部視為沒有勿擾時段，並在日誌留下 `warn` 記錄告訴你哪個值被忽略了。理由很直接：解析器的 bug 不該讓一個部署整晚收不到通知。設定寫錯的代價是「照常收到通知」，而非「靜悄悄什麼都收不到」。
...
- **同一個收件目的地的 L1 通知合併成一則**（`🌙 勿擾時段收到 3 則通知：` + 編號清單），而非三則分開的訊息。
- **決定卡單獨投遞**，因為每張卡有自己的按鈕。
- 佇列有雙重上限：最多 500 則、超過 36 小時的直接丟棄（一天半前的「任務完成了」已經算不上新聞）。**丟棄一定會寫 `warn` 日誌**，不會無聲消失。
...
會被靜默的條件必須攤在使用者看得到的地方，這是硬要求，算不上加分項。
...
全部歸零的一天，**不發訊息**，也不會發一則「今日無事」。一個你每天都可以不看的摘要，會訓練你連該看的那天也不看。
...
- **同一張卡按兩次只算一次行動**（用決定 id 比對，非單純計數），所以行動率不會超過 100%。
- **被拒絕的按壓不算行動**：按了但被權限或狀態擋掉，是失敗的互動，構不成「這則通知有用」的證據。
- 純週知類（沒有按鈕可按）會回報 `actionable: 0` 且**永遠不會被標記 broken**。說「FYI 的行動率是 0%」是同義反覆，稱不上發現。

儀表板圖表已實作，位置見「儀表板介面位置」一節。
...
| 通知成效（`notify.stats` 行動率） | 分析報表頁 →「通知成效」卡 | 依「行動率量測」一節的 SRE 50% 判準列出各通知類型的推播數／行動率，行動率過低的類別長條標紅並附一句提示文字（不只靠顏色，避免色弱或黑白列印看不出來）；純週知類（沒有按鈕可按）永遠不會被標記壞掉。 |
