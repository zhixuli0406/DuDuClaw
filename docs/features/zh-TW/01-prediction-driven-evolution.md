...
預測引擎要比較「預期結果」與「實際結果」之前，得先搞懂這次對話*到底發生了什麼*。**ConversationOutcome** 模組會沿三個維度為每次對話分類——全程不呼叫 LLM：
...
當一次預測失準（顯著或嚴重等級的誤差），細節會被記錄進 **MistakeNotebook**——一份持續累積的失敗模式紀錄：
...
這讓演化引擎不會對路上每一個小坑洞都反應過度，而是等到證據累積到足以支撐一次有意義的改動——結果是更穩定、品質更高的演化。
