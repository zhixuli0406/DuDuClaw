...
| Dispatch failure (the work message never reached a runtime — CLI missing, local engine down, credentials refused) | slot and edition lease released on the next tick; back-off 60 → 120 → 240 s; 3 consecutive failures | `needs_human` (`infra`), error text on the task timeline |
