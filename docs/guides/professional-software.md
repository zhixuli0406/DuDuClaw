...
本指南說明如何讓 DuDuClaw 的 AI 員工操作 Adobe Photoshop 與 AutoCAD。DuDuClaw 不自寫這些軟體的驅動，而是掛載社群維護的 MCP server，透過 per-agent `.mcp.json` 接上，再用 capability 治理圍住高風險工具。這樣新增一個專業軟體不必等 DuDuClaw 出新版，換一個 MCP server 就換一套能力。
