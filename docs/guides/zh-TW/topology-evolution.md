...
SOUL.md，細節見 `docs/architecture/evolution-engine.md` 第十二章）只優化「節點」，
也就是每個 agent 的 prompt/行為規則。多 agent 之間的「邊」，也就是 `reports_to`
階層把某類任務路由給誰，一直是寫死的。D5 讓這條邊變成可演化的對象，但每一次改動
都必須經過人工核准，機器只負責提案與證據收集。
...
2. **提案**（不是直接改動）
   ...
   task id）。沒有合格 sibling 就不提案，因為空結果優於假結果。每個 tick 最多提一筆。
