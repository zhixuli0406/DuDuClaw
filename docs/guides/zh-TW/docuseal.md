...
[DocuSeal](https://github.com/docusealco/docuseal) 是開源的 DocuSign 替代品（cloud 或 self-hosted）。DuDuClaw 提供 `duduclaw-docuseal-mcp`，一個開源的 MCP stdio wrapper，讓 agent 走「生成合約 → 寄送簽署 → 查狀態 → 取回簽署檔」全流程。
