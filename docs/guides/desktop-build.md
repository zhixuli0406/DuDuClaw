...
The DMG window itself is styled via `bundle.macOS.dmg` (custom background,
window size, icon positions). The background is `src-tauri/dmg/background.png`,
regenerated by `scripts/desktop/gen-dmg-background.py` (Pillow; uses Heiti TC for
zh-TW text since PingFang isn't on every macOS). Edit the script, not the PNG.
