...
派工引擎自 v1.59 起預設開啟（沒有目標任務時只做週期性的 SQLite 輪詢，驗收判官只在任務真正進入 review 時才花一次 LLM 呼叫），一問一答對話完全不受影響。不想要時在 `config.toml` 設 `[dispatch] enabled = false`，或到儀表板「設定 → 自動化」關閉「派工引擎」開關（免重啟熱生效）。
...
任務轉「需人工」時（達派工上限 / 牆鐘超時 / 連續兩輪駁回且回饋雷同 / 驗收判官在重試預算耗盡時仍不通過 / 上游依賴子任務卡住而繼承升級），會推送四顆按鈕到 AI 員工的控制頻道：
