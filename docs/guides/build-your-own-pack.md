...
> 這是 tutorial；欄位完整參考見 [features/32-expert-packs.md](../features/32-expert-packs.md)。
...
`expert.toml` 最小內容（欄位以 [features/32](../features/32-expert-packs.md) 為準）：
...
- **需求宣告**：`[expert.requires]` 的 `env`（需要的環境變數）與 `bins`（需要的外部指令）讓安裝者在裝之前就知道前置條件，而不是裝完才踩雷。
